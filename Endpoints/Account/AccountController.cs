...
namespace IdentityServer4.Quickstart.UI
{
  ...
  public class AccountController : Controller
  {
    ...
    private readonly ResourceOwnerPasswordValidator _resourceOwnerPasswordValidation;
  ...
  //    HttpContext.Response.Cookies.Append("returnUrl", returnUrl);
    ...
    public async Task<IActionResult> Login(LoginInputModel model, string button)
    {
      ...
      if (ModelState.IsValid)
      {
        ...
        ValidationContext = await _resourceOwnerPasswordValidation.ValidatePassword(ValidationContext);
        ...
        if (ValidationContext.Result != null)
        {
          if (ValidationContext.Result.Subject != null)
          {
            ...
            {
              var user = _userRepository.Users.FirstOrDefault(x => x.Username == model.Username);
              ...
              //var claims = await new ProfileService().PopulateClaims(user);

              var claims = ProfileService.GetUserClaims(user);
              // issue authentication cookie with subject ID and username
              await HttpContext.SignInAsync(user.UsersId.ToString(), user.Username, "ssodb", props, claims.ToArray());
            }
